...
export async function GET(req: NextRequest, { params }: { params: { websiteId: string } }) {
    ...
    try {
        ...
        try {
            ...
            await cloneRepository(website.githubRepositoryUrl, cloneDirPath);
            ...
            response.headers.set("Content-Disposition", `attachment; filename="website-code-${website.siteName}.zip"`);
        ...
        return NextResponse.json(
            { error: "Failed to download website code", message: error instanceof Error ? error.message : "Unknown error" },
            { status: 500 },
        );
    }
}
