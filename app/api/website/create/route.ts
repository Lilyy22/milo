...
export async function POST(req: NextRequest) {
    ...
    const isWebsiteExists = await prisma.website.findFirst({ where: { clientId } });
    if (isWebsiteExists) {
        return NextResponse.json(
            {
                ...
                message: "Website already exists for this client. Redirecting to projects page.",
            },
            { status: 302 },
        );
    }
    ...
    try {
        const projectName = `website-${client.clientName.toLowerCase().replace(/[^a-z0-9-]/g, "-")}`;
        ...
        const { newWebsiteDir, newConstantsPath, newLayoutPath } = await websiteGenerator.generateWebsite(websiteData, website.id.toString());
        ...
        try {
            const { vercelProjectId, vercelDeploymentId } = await vercelClient.deployToVercel({ repositoryUrl, projectName });
        ...
        return NextResponse.json(
            { error: "Failed to generate, deploy or archive website", message: error instanceof Error ? error.message : "Unknown error" },
            { status: 500 },
        );
    } finally {
