...
type FilesToCleanup = { constantsPath?: string; websitePath?: string; websiteArchivePath?: string; layoutPath?: string };
...
export async function PUT(req: NextRequest) {
    ...
    const isWebsiteExists = await prisma.website.findUnique({ where: { clientId } });
    if (!isWebsiteExists || !isWebsiteExists.githubRepositoryUrl) {
        return NextResponse.json({ error: "Website not found or GitHub repository URL not available" }, { status: 404 });
    }

    console.log("Starting website update process...");

    try {
        const { newWebsiteDir, newConstantsPath, newLayoutPath } = await websiteGenerator.generateWebsite(websiteData, isWebsiteExists.id.toString());
        ...
        return NextResponse.json(
            { error: "Failed to update website", message: error instanceof Error ? error.message : "Unknown error" },
            { status: 500 },
        );
    } finally {
