...
import {
  ...
  postRequestSchema,
  ClientDataType,
} from "@/schemas/generate-content";

class ContentGenerator {
  private readonly promptBuilders: Record<
    string,
    { additionalSystemPrompt: string; schema: z.ZodType<any> }
  >;
  ...
  async generateAiResponse(
    ...
    try {
      const { additionalSystemPrompt, schema } =
        this.promptBuilders[page as keyof typeof this.promptBuilders];
  ...
  private buildHomePrompt(): {
    ...
    return {
      additionalSystemPrompt: AI_PROMPTS.HOME_PAGE,
      schema: homePageContentSchema,
    };
  }

  private buildAboutPrompt(): {
    additionalSystemPrompt: string;
    schema: z.ZodType<any>;
  } {
    return {
      additionalSystemPrompt: AI_PROMPTS.ABOUT_PAGE,
  ...
  private buildBlogPrompt(): {
    ...
    return {
      additionalSystemPrompt: AI_PROMPTS.BLOG_PAGE,
      schema: z.string().describe("Blog page content"),
    };
  }
  ...
  private buildServicesPrompt(): {
    ...
    return {
      additionalSystemPrompt: AI_PROMPTS.NEWS_PAGE,
      schema: z.string().describe("Services/news page content"),
    };
  }

  private buildContactPrompt(): {
    additionalSystemPrompt: string;
    schema: z.ZodType<any>;
  } {
    return {
      additionalSystemPrompt:
        "Create a contact page section with content about how to get in touch.",
...
export async function POST(req: Request) {
  try {
    ...
    const clientData = await getClientById(strClientId);
    if (!clientData || !clientData.bio || typeof clientData.bio !== "string") {
      return NextResponse.json(
        { error: "Invalid client data: biography is required" },
        { status: 400 }
      );
    }
    ...
    if (Object.keys(content).length === 0) {
      return NextResponse.json(
        { error: "Failed to generate content for any of the requested pages" },
        { status: 500 }
      );
    }
