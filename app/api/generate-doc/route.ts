...
async function prepareAssistantAndFiles(files: { content: string, filename: string }[], assistantId: string): Promise<{ assistant: any, thread: any, vectorStore: any }> {
    ...
    // Prepare all files
    const openAIFiles = await Promise.all(files.map(async (file) => {
        console.log(`File size (${file.filename}): ${file.content.length} bytes (${(file.content.length / (1024 * 1024)).toFixed(2)} MB)`);
    ...
    const uploadResult = await openai.beta.vectorStores.fileBatches.uploadAndPoll(vectorStore.id, {
        files: openAIFiles
    });
...
async function generateWithAssistant(data: any, assistantId: string): Promise<string> {
    ...
    const firstResponse = responseMessages.data.find(m => m.role === 'assistant');
    
    if (!firstResponse || !firstResponse.content[0] || firstResponse.content[0].type !== 'text') {
        throw new Error('No valid response from assistant');
    }

    fullContent = firstResponse.content[0].text.value.replace(/Part \d+:?\s*/gi, '');
    ...
    if (fullContent.split(/\s+/).length < targetWordCount) {
        ...
        const secondResponse = (await openai.beta.threads.messages.list(thread.id))
            .data.find(m => m.role === 'assistant');

        if (secondResponse?.content[0]?.type === 'text') {
            const part2Content = secondResponse.content[0].text.value.replace(/Part \d+:?\s*/gi, '');
            fullContent += '\n\n' + part2Content;
        }
    }

    // Apply name rules once at the end
    fullContent = enforceNameRules(fullContent, data.clientName, data.isBusinessName);

    console.log(`Final word count: ${fullContent.split(/\s+/).length}`);
    return fullContent;
}

async function saveToGoogleDocs(content: string, folderId: string, fileName: string, newOwnerEmail: string): Promise<string> {
  const maxRetries = 3;
  let retryCount = 0;
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      const finalContent = cleanedContent.replace(/\*\*/g, '').replace(/###\s*/g, '');
      await docs.documents.batchUpdate({
        documentId,
        requestBody: {
          requests: [
      ...
      if (error instanceof Error && 
        ...
        if (status === 502) {
          ...
          if (retryCount < maxRetries) {
            ...
            console.log(`Retrying saveToGoogleDocs (attempt ${retryCount + 1}) after ${delay}ms`);
...
export async function POST(request: NextRequest) {
  try {
    const { stageId, clientId, category, folderId, qaId, isBusinessName = false } = await request.json();
    ...
    if (!clientData || !stageData) {
      return NextResponse.json({ success: false, error: 'Client data or Stage data not found' });
    }
    ...
    if (qaId) {
      ...
      if (!selectedQa) {
        return NextResponse.json({ success: false, error: 'Selected QA document not found' });
      }
    ...
    console.log(`Generated article with ${articleContent.split(/\s+/).length} words`);

    // Ensure word count is 1000 or more
    if (articleContent.split(' ').length < 900) {
      throw new Error('Article content does not meet the minimum word count of 1000.');
    }

    // Save the article to Google Docs in the client folder
    let documentId;
    try {
      documentId = await saveToGoogleDocs(articleContent, clientFolderId!, stageData.name ?? 'Untitled', newOwnerEmail!);
    } catch (error) {
      console.error('Error saving to Google Docs:', error);
      return NextResponse.json({ success: false, error: 'Failed to save document to Google Docs. Please try again later.' });
    }

    // Update the stage with the Google Docs URL
    const updatedStage = await prisma.stage.update({
      where: { id: stageId },
      data: { docUrl: `https://docs.google.com/document/d/${documentId}/edit`, status: 'Ready'},
    });
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!stageData || !stageData.docUrl) {
      return NextResponse.json({ success: false, error: 'Stage data or Document URL not found' });
    }
...
function enforceNameRules(content: string, fullName: string, isBusinessName: boolean = false): string {
  ...
  if (isBusinessName) {
    ...
    if (!title.includes(fullName)) {
      title = title.replace(new RegExp(escapeRegExp(fullName.split(' ').pop() || ''), 'i'), fullName);
    }
  ...
  if (!firstParagraph.includes(fullName)) {
    body = body.replace(firstParagraph, firstParagraph.replace(new RegExp(`\\b${lastName}\\b`, 'i'), fullName));
  }
  ...
  if (!lastParagraph.includes(fullName)) {
    lastParagraph = lastParagraph.replace(new RegExp(`\\b${lastName}\\b`, 'i'), fullName);
...
function getStrategyForAssistant(assistantId: string): ArticleGenerationStrategy {
    switch (assistantId) {
