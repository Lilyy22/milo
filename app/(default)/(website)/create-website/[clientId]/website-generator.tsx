...
import {
  ...
  createWebsite,
} from "@/app/(default)/(website)/actions/website";
...
export const WebsiteGenerator = ({ clientData }: WebsiteGeneratorProps) => {
  ...
  const [formData, setFormData] = useState<CreateWebsiteDataType>(() =>
    getDefaultCreateWebsiteData(clientData)
  );
  ...
  const handleGenerateContent = async () => {
    ...
    if (pages.length === 0)
      return toast.error(
        "Please select at least one page to generate content for"
      );

    setIsGenerating(true);

    return toast.promise(
      generateWebsiteContent(clientData.id, pages).then((data) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const websiteData = {
      ...
      ...(generatedContent.about && {
        aboutPageContent: generatedContent.about,
      }),
      ...
      ...(generatedContent.contact && {
        contactPageContent: generatedContent.contact,
      }),
    };

    return toast.promise(
      createWebsite(clientData.id, websiteData).then(() =>
