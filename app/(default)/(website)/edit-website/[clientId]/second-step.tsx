...
export const SecondStep = ({ formData, generatedContent, setGeneratedContent, setCurrentStep }: SecondStepProps) => {
	const [selectedPage, setSelectedPage] = useState<string>(formData.pages[0]?.replace('/', '') || 'home')
	const handlePageChange = (page: string) => setSelectedPage(page.replace('/', ''))

	const selectedPageContent = selectedPage && generatedContent[selectedPage]
				...
				<div className="space-y-4 bg-white dark:bg-slate-800 rounded-lg p-3 shadow-sm border border-slate-200 dark:border-slate-700">
					<div className="flex items-center text-slate-700 dark:text-slate-200">
						...
						{formData.pages.map(page => (
							<span key={page} className="inline-block px-2 py-1 bg-slate-200 dark:bg-slate-700 rounded ml-2 capitalize">
								{PAGES_NAVIGATION[page as keyof typeof PAGES_NAVIGATION]?.label || page.replace('/', '')}
							</span>
						))}
					</div>
				</div>
