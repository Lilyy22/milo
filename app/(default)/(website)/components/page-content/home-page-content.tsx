  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        <h4 className="font-medium text-xl text-slate-800 dark:text-slate-100">
          Hero Block
        </h4>
        <div className="space-y-5">
          <EditableTextField
            label="Section Title"
            value={content.heroBlock?.title || ""}
            onChange={(value) => updateContent("heroBlock.title", value)}
          />
          <EditableTextField
            label="Text"
            value={content.heroBlock?.subtitle || ""}
            onChange={(value) => updateContent("heroBlock.subtitle", value)}
            multiline
          />
        </div>
      </div>
      ...
      <div className="space-y-6">
        <h4 className="font-medium text-xl text-slate-800 dark:text-slate-100">
          {content.keysBlock?.title || "Key Features Block"}
        </h4>
        <div className="space-y-6">
          <EditableTextField
            label="Section Title"
      ...
      {content.keysBlock && (
        <div>
          <h2 className="text-2xl font-bold mb-6 text-slate-800 dark:text-slate-100">
            {content.keysBlock.title}
          </h2>
          <ul className="grid grid-cols-3 gap-6 mt-10 mb-6">
            {content.keysBlock.items?.map((item: string, idx: number) => (
              <li
                className="p-[8px_8px_8px_16px] border-l-2 border-black/80 dark:border-white/80"
                key={idx}
              >
                {item}
              </li>
