...
import { EditableTextField, EditableObjectArray } from '../editable-fields';
...
export const AboutPageContent = ({
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        <h4 className="font-medium text-xl text-slate-800 dark:text-slate-100">{content.keysBlock?.title || 'Values & Principles'}</h4>
        <div className="space-y-6">
          <EditableTextField
            label="Section Title"
...
export const renderAboutPageContent = (content: any) => {
  ...
  return (
    <div className="space-y-12">
      ...
      {content.keysBlock && (
        <div>
          <h2 className="text-2xl font-bold mb-3 text-slate-800 dark:text-slate-100">{content.keysBlock.title}</h2>
          <ul className="grid grid-cols-3 gap-6 mt-10 mb-6">
            {content.keysBlock.items?.map((item: { name: string, description: string }, idx: number) => (
              <li className="p-[8px_8px_8px_16px] border-l-2 border-black/80 dark:border-white/80" key={idx}>
                <h3 className="font-bold text-xl text-slate-800 dark:text-slate-100 mb-2">{item.name}</h3>
