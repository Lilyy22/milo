...
class VercelSDK {
  ...
  async createVercelProject(projectName: string, repositoryUrl: string) {
    const { githubUsername, repositoryName } =
      this.getGitHubInfo(repositoryUrl);
    ...
    try {
      ...
      const deployment = await this.vercel.deployments.createDeployment({
        requestBody: {
          ...
          projectSettings: {
            framework: "nextjs", // Specify the framework
            buildCommand: "npm run build", // Build command
            outputDirectory: ".next", // Output directory for Next.js
          },
        },
      });
      ...
      return {
        ...
        vercelDeploymentId: deployment.url,
