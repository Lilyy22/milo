...
class GitHubClient {
    ...
    constructor(private readonly githubToken: string = process.env.GITHUB_TOKEN!) {
    ...
    public async createRepository(repositoryName: string, description: string) {
        try {
            const { data: repository } = await this.octokit.repos.createForAuthenticatedUser({
                name: repositoryName,
                description,
                private: true,
                auto_init: false,
            ...
            return { repositoryUrl: repository.html_url, cloneUrl: repository.clone_url };
        } catch (error) {
    ...
    private async initGitAndPush(directory: string, repositoryUrl: string) {
        const repoUrlWithToken = repositoryUrl.replace("https://", `https://x-access-token:${this.githubToken}@`);
        ...
        try {
            ...
            await git.raw(["branch", "-M", this.DEFAULT_BRANCH]).catch(() => git.checkoutLocalBranch(this.DEFAULT_BRANCH));
            ...
            throw new Error(`Git operations failed: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }

    public async deployWebsite(websiteDir: string, repositoryUrl: string) {
        try {
            console.log(`Deploying website from ${websiteDir} to GitHub repository: ${repositoryUrl}`);
            ...
            throw new Error(`Failed to deploy to GitHub: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }

    private async checkFileNeedsUpdate({ owner, repoName, filePath, newContent }: CheckFileNeedsUpdateParams) {
        try {
            ...
            if (!Array.isArray(existingFile) && existingFile.type === "file") {
                ...
                return { needsUpdate: existingContent.trim() !== newContent.trim(), sha };
            }
        } catch (error) {
    ...
    private async updateFileInRepository({ owner, repoName, filePath, content, commitMessage, sha }: UpdateFileInRepositoryParams) {
        ...
        console.log(`File ${filePath} ${sha ? "updated" : "created"} in repository ${owner}/${repoName}`);
    }

    public async updateRepository({ websiteDir, repositoryUrl }: { websiteDir: string; repositoryUrl: string }) {
        try {
            ...
            try {
                ...
                await git.add("src/app");
                await git.add("src/constants");
                ...
                if (!status.isClean()) {
                    ...
                    console.log(`Successfully updated src/app and src/constants in repository ${repositoryUrl}`);
                } else {
    ...
    public async cloneRepository(repositoryUrl: string, targetDir: string): Promise<string> {
        try {
