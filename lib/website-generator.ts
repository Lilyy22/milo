...
class WebsiteGenerator {
  ...
  async createConstantsFile(
    websiteData: CreateWebsiteDataType,
    websiteId: string
  ): Promise<string> {
    ...
    export const ABOUT_PAGE_CONTENT = ${JSON.stringify(
      aboutPageContent,
      null,
      2
    )};`;
    ...
    const constantsPath = path.join(
      FOR_WEBSITES_DIR,
      `constants_${websiteId}_${Date.now()}.js`
    );
  ...
  async generateLayoutFile(
    ...
    const template =
      websiteData.template === "BASIC" ? "template" : "elitetemplate";
    ...
    const newLayoutPath = path.join(
      FOR_WEBSITES_DIR,
      `layout_${websiteId}_${Date.now()}.js`
    );
  ...
  async generateWebsite(websiteData: CreateWebsiteDataType, websiteId: string) {
    ...
    const createdWebsiteDir = path.join(
      NEW_WEBSITES_DIR,
      `website_${websiteId}_${Date.now()}`
    );
    await fs.ensureDir(createdWebsiteDir);

    await this.copyTemplate(createdWebsiteDir, TEMPLATE_DIR);

    const tempConstantsPath = await this.createConstantsFile(
      websiteData,
      websiteId
    );
    await this.replaceConstantsFile(tempConstantsPath, createdWebsiteDir);

    const tempLayoutPath = await this.generateLayoutFile(
      websiteData,
      websiteId
    );
    await this.replaceLayoutFile(tempLayoutPath, createdWebsiteDir);

    if (websiteData.template === "BASIC") {
  ...
  // copy the template file to any folder
  private async copyTemplate(
    createdWebsiteDir: string,
    TEMPLATE_DIR: string
  ): Promise<void> {
    if (!(await fs.pathExists(TEMPLATE_DIR)))
      throw new Error(`Template directory does not exist: ${TEMPLATE_DIR}`);

    await fs.copy(TEMPLATE_DIR, createdWebsiteDir);
    console.log("Template copied successfully");
  }

  private async replaceConstantsFile(
    tempConstantsPath: string,
    createdWebsiteDir: string
  ): Promise<void> {
    const destConstantsPath = path.join(
      createdWebsiteDir,
  ...
  private async replaceLayoutFile(
    tempLayoutPath: string,
    createdWebsiteDir: string
  ): Promise<void> {
  ...
  async cleanup({
    ...
    try {
      if (constantsPath && (await fs.pathExists(constantsPath)))
        await fs.remove(constantsPath);
      if (websitePath && (await fs.pathExists(websitePath)))
        await fs.remove(websitePath);
      if (websiteArchivePath && (await fs.pathExists(websiteArchivePath)))
        await fs.remove(websiteArchivePath);
