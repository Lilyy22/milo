...
export default function DropdownProfile({ align }: {
  ...
  const {session, loading} = useSessionContext() as any
    ...
    <Menu as="div" className="relative inline-flex">
      ...
      >
        <div className="pt-0.5 pb-2 px-3 mb-1 border-b border-slate-200 dark:border-slate-700">
          <div className="font-medium text-slate-800 dark:text-slate-100">{session?.user?.email}</div>
        ...
        <Menu.Items as="ul" className="focus:outline-none">
          ...
          <Menu.Item as="li">
            {({ active }) => (
              <Link href="#0" onClick={() => signOut()} className={`font-medium text-sm flex items-center py-1 px-3 ${active ? 'text-indigo-600 dark:text-indigo-400' : 'text-indigo-500'}`}>
                Sign Out
              </Link>
            )}
          </Menu.Item>
